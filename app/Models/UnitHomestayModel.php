...
class UnitHomestayModel extends Model
{
    ...
    // public function get_homestay_by_prioritas($checkInDate)
    // {
    //     $currentDate = $checkInDate;

    //     $query = $this->db->table('unit_homestay uh')
    //         ->select('h.name,uh.homestay_id, uh.unit_type, uh.unit_number, COUNT(dr.reservation_id) AS total_reservations')
    //         ->select('AVG(dr.rating) AS avg_rating')
    //         ->select('COALESCE(SUM(dr.unit_guest), 0) AS unit_guest')
    //         ->select('COALESCE(SUM(uh.capacity), 0) AS unit_capacity')
    //         ->join('homestay h', 'h.id = uh.homestay_id')
    //         ->join('detail_reservation dr', 'uh.homestay_id = dr.homestay_id AND uh.unit_type = dr.unit_type AND uh.unit_number = dr.unit_number AND dr.date = ' . $this->db->escape($currentDate), 'left')
    //         ->groupBy('uh.homestay_id, uh.unit_type, uh.unit_number')
    //         ->orderBy('uh.homestay_id ASC')
    ...
    // public function unit_tersedia($homestay_id){
    //     $query = $this->db->table($this->table)
    //     ->select('unit_homestay.homestay_id, unit_homestay.unit_type, unit_homestay.unit_number, COUNT(detail_reservation.reservation_id) AS total_reservations')
    //     ->join('homestay_unit_type', 'unit_homestay.unit_type = homestay_unit_type.id')
    //     ->join('gallery_unit', 'unit_homestay.homestay_id = gallery_unit.homestay_id')
    //     ->join('detail_reservation', 'unit_homestay.homestay_id = detail_reservation.homestay_id AND unit_homestay.unit_type = detail_reservation.unit_type AND unit_homestay.unit_number = detail_reservation.unit_number AND (YEAR(detail_reservation.date) = YEAR(CURRENT_DATE()) OR detail_reservation.date IS NULL)', 'left')
    //     ->where('unit_homestay.homestay_id', $homestay_id)
    //     ->where('unit_homestay.unit_type = gallery_unit.unit_type')
    //     ->where('unit_homestay.unit_number = gallery_unit.unit_number')
    ...
    // public function unit_tersedia($homestay_id, $checkInDate, $totalPeople)
    // {
    //     $query = $this->db->table($this->table)
    //         ->select('unit_homestay.homestay_id, unit_homestay.unit_type, unit_homestay.unit_number, COUNT(detail_reservation.reservation_id) AS total_reservations, detail_reservation.date')
    //         ->join('homestay_unit_type', 'unit_homestay.unit_type = homestay_unit_type.id')
    //         ->join('gallery_unit', 'unit_homestay.homestay_id = gallery_unit.homestay_id')
    //         ->join('detail_reservation', 'unit_homestay.homestay_id = detail_reservation.homestay_id AND unit_homestay.unit_type = detail_reservation.unit_type AND unit_homestay.unit_number = detail_reservation.unit_number AND (YEAR(detail_reservation.date) = YEAR(' . $this->db->escape($checkInDate) . ') OR detail_reservation.date IS NULL)', 'left')
    //         ->where('unit_homestay.homestay_id', $homestay_id)
    //         ->where('unit_homestay.unit_type = gallery_unit.unit_type')
    //         ->where('unit_homestay.unit_number = gallery_unit.unit_number')
    //         // ->where('unit_homestay.capacity >=', $totalPeople) // Pastikan kapasitas unit mencukupi untuk jumlah tamu
    //         ->groupby('unit_homestay.homestay_id, unit_homestay.unit_type, unit_homestay.unit_number, detail_reservation.date')
    //         ->orderby('total_reservations', 'ASC')
    //         ->where('detail_reservation.date IS NULL')
    //         ->get();
    ...
    // {
    //     $query = $this->db->table($this->table)
    //         ->select('unit_homestay.homestay_id, gallery_unit.url , unit_homestay.capacity as room_capacity, unit_homestay.unit_type, unit_homestay.description, unit_homestay.price, unit_homestay.unit_name, unit_homestay.unit_number, unit_homestay.capacity, COUNT(detail_reservation.reservation_id) AS total_reservations, detail_reservation.date')
    //         ->join('homestay_unit_type', 'unit_homestay.unit_type = homestay_unit_type.id')
    //         ->join('gallery_unit', 'unit_homestay.homestay_id = gallery_unit.homestay_id')
    //         ->join('detail_reservation', 'unit_homestay.homestay_id = detail_reservation.homestay_id AND unit_homestay.unit_type = detail_reservation.unit_type AND unit_homestay.unit_number = detail_reservation.unit_number AND (YEAR(detail_reservation.date) = YEAR(' . $this->db->escape($checkInDate) . ') OR detail_reservation.date IS NULL)', 'left')
    //         ->where('unit_homestay.homestay_id', $homestay_id)
    ...
    //         // ->where('unit_homestay.capacity >=', $totalPeople) // Pastikan kapasitas unit mencukupi untuk jumlah tamu
    //         ->where('detail_reservation.date IS NULL')
    //         ->groupby('unit_homestay.homestay_id, unit_homestay.unit_type, unit_homestay.unit_number, detail_reservation.date, gallery_unit.url')
    //         ->orderby('total_reservations', 'ASC')
    //         ->get();
    ...
    // public function unit_tersedia($homestay_id, $unit_type, $unit_number, $checkInDate, $totalPeople)
    // {
    //     // Cek kapasitas aula yang tersedia pada tanggal yang sama
    //     $available_lecture_hall = $this->db->table($this->table)
    //         ->select('unit_homestay.homestay_id, unit_homestay.capacity')
    //         ->join('homestay_unit_type', 'unit_homestay.unit_type = homestay_unit_type.id')
    //         ->join('detail_reservation', 'unit_homestay.homestay_id = detail_reservation.homestay_id AND unit_homestay.unit_type = detail_reservation.unit_type AND unit_homestay.unit_number = detail_reservation.unit_number AND detail_reservation.date = ' . $this->db->escape($checkInDate), 'left')
    //         ->where('unit_homestay.homestay_id', $homestay_id)
    //         ->where('unit_homestay.unit_type', 'T2') // Ganti dengan tipe unit untuk aula
    //         ->where('unit_homestay.unit_number', $unit_number)
    //         ->where('detail_reservation.date', $checkInDate)
    //         ->groupby('unit_homestay.homestay_id')
    //         ->get()
    //         ->getRow();

    //     // Jika kapasitas aula masih tersedia pada tanggal yang sama, prioritas penggunaan aula
    //     if ($available_lecture_hall && $available_lecture_hall->capacity > 0) {
    //         $query = $this->db->table($this->table)
    //             ->select('unit_homestay.homestay_id, gallery_unit.url, unit_homestay.capacity as room_capacity, unit_homestay.unit_type, unit_homestay.description, unit_homestay.price, unit_homestay.unit_name, unit_homestay.unit_number, unit_homestay.capacity, COUNT(detail_reservation.reservation_id) AS total_reservations')
    //             ->join('homestay_unit_type', 'unit_homestay.unit_type = homestay_unit_type.id')
    //             ->join('gallery_unit', 'unit_homestay.homestay_id = gallery_unit.homestay_id')
    ...
    //             ->get();
    //     } else {
    //         // Jika tidak ada kapasitas tersisa pada aula, gunakan logika default
    //         $query = $this->db->table($this->table)
    //             ->select('unit_homestay.homestay_id, gallery_unit.url, unit_homestay.capacity as room_capacity, unit_homestay.unit_type, unit_homestay.description, unit_homestay.price, unit_homestay.unit_name, unit_homestay.unit_number, unit_homestay.capacity, COUNT(detail_reservation.reservation_id) AS total_reservations')
    //             ->join('homestay_unit_type', 'unit_homestay.unit_type = homestay_unit_type.id')
    //             ->join('gallery_unit', 'unit_homestay.homestay_id = gallery_unit.homestay_id')
    ...
    // {
    //     $query = $this->db->table($this->table)
    //         ->select('unit_homestay.homestay_id, gallery_unit.url, unit_homestay.capacity as room_capacity, unit_homestay.unit_type, unit_homestay.description, unit_homestay.price, unit_homestay.unit_name, unit_homestay.unit_number, unit_homestay.capacity, COUNT(detail_reservation.reservation_id) AS total_reservations,  COALESCE(SUM(detail_reservation.unit_guest), 0) AS unit_guest,
    //         COALESCE(SUM(unit_homestay.capacity - detail_reservation.unit_guest), 0) AS unit_remaining')
    //         ->join('gallery_unit', 'unit_homestay.homestay_id = gallery_unit.homestay_id')
    //         ->join('detail_reservation', 'unit_homestay.homestay_id = detail_reservation.homestay_id 
    //         AND unit_homestay.unit_type = detail_reservation.unit_type 
    //         AND unit_homestay.unit_number = detail_reservation.unit_number 
    //         AND detail_reservation.date = ' . $this->db->escape($checkInDate), 'left')
    //         ->where('unit_homestay.homestay_id', $homestay_id)
    //         ->where('unit_homestay.unit_type', $unit_type)
    //         ->where('unit_homestay.unit_number', $unit_number)
    //         ->where('unit_homestay.unit_number = gallery_unit.unit_number')
    //         ->groupby('unit_homestay.homestay_id, unit_homestay.unit_type, unit_homestay.unit_number, gallery_unit.url')
    //         ->having('unit_remaining >', 0)
    //         ->orHaving('unit_remaining IS NULL')
    //         ->orderby('total_reservations', 'DESC')
    //         ->get();
    ...
    // public function unit_tersedia2($homestay_id, $unit_type, $unit_number, $checkInDate, $totalPeople)
    // {
    //     $query = $this->db->table($this->table)
    //         ->select('unit_homestay.homestay_id, gallery_unit.url, unit_homestay.capacity as room_capacity, unit_homestay.unit_type, unit_homestay.description, unit_homestay.price, unit_homestay.unit_name, unit_homestay.unit_number, unit_homestay.capacity, COUNT(detail_reservation.reservation_id) AS total_reservations,  COALESCE(SUM(detail_reservation.unit_guest), 0) AS unit_guest,
    //     COALESCE(SUM(unit_homestay.capacity - detail_reservation.unit_guest), 0) AS unit_remaining')
    ...
    //         ->where('unit_homestay.unit_number = gallery_unit.unit_number')
    //         ->groupby('unit_homestay.homestay_id, unit_homestay.unit_type, unit_homestay.unit_number, gallery_unit.url')
    //         ->having('unit_remaining >=', $totalPeople)
    //         ->orderby('total_reservations', 'DESC');

    //     $result = $query->get()->getResultArray(); // Menggunakan getResultArray() untuk mendapatkan array hasil

    //     return $result;
    // }
    // public function unit_tersedia_lagi($homestay_id, $unit_type, $unit_number, $checkInDate, $totalPeople, $selectedUnits = [])
    // {
    //     $sql = "SELECT 
    //                 unit_homestay.homestay_id, 
    //                 gallery_unit.url, 
    //                 unit_homestay.capacity AS room_capacity, 
    //                 unit_homestay.unit_type, 
    //                 unit_homestay.description, 
    //                 unit_homestay.price, 
    //                 unit_homestay.unit_name, 
    //                 unit_homestay.unit_number, 
    //                 unit_homestay.capacity, 
    //                 COUNT(detail_reservation.reservation_id) AS total_reservations,  
    //                 COALESCE(SUM(detail_reservation.unit_guest), 0) AS unit_guest,
    //                 COALESCE(SUM(unit_homestay.capacity - detail_reservation.unit_guest), 0) AS unit_remaining
    //             FROM unit_homestay
    //             JOIN gallery_unit ON unit_homestay.homestay_id = gallery_unit.homestay_id
    //             LEFT JOIN detail_reservation ON unit_homestay.homestay_id = detail_reservation.homestay_id 
    //                 AND unit_homestay.unit_type = detail_reservation.unit_type 
    //                 AND unit_homestay.unit_number = detail_reservation.unit_number 
    //                 AND detail_reservation.date = ?
    //             WHERE unit_homestay.homestay_id = ?
    //                 AND unit_homestay.unit_type = ?
    //                 AND unit_homestay.unit_number = ?
    //                 AND unit_homestay.unit_number = gallery_unit.unit_number
    //                 AND unit_homestay.unit_number NOT IN (" . implode(",", array_fill(0, count($selectedUnits), "?")) . ")
    //             GROUP BY unit_homestay.homestay_id, unit_homestay.unit_type, unit_homestay.unit_number, gallery_unit.url
    //             HAVING unit_remaining >= ?
    //             ORDER BY total_reservations DESC";

    //     $selectedUnits[] = $checkInDate;
    //     $selectedUnits[] = $homestay_id;
    //     $selectedUnits[] = $unit_type;
    //     $selectedUnits[] = $unit_number;
    //     $selectedUnits[] = $totalPeople;

    //     return $this->db->query($sql, $selectedUnits);
    // }
    ...
    // {
    //     $query = $this->db->table($this->table)
    //         ->select('unit_homestay.*', 'homestay_unit_type.name as unit_type_name')
    //         ->join('homestay_unit_type', 'unit_homestay.unit_type = homestay_unit_type.id')
    //         ->where('homestay_id', $homestay_id);

    //     // Check if there are reservations on the specified date
    //     $reservationQuery = $this->db->table('detail_reservation')
    //         ->where('date', $checkInDate)
    //         ->where('homestay_id', $homestay_id)
    //         ->get();


    //     if ($reservationQuery->getNumRows() > 0) {
    //         // If there are reservations, exclude those units
    //         $reservedUnitIds = array_column($reservationQuery->getResultArray(), 'unit_homestay_id');
    //         $query->whereNotIn('unit_homestay.id', $reservedUnitIds);
    //     }
